...
# Note: We will just test that the string returned includes the 6 values,
    ...
    def __str__(self) -> str:
        ...
        """
        return (f'Average: {self.avg_temp} Low: {self.low_temp} High: '
                f'{self.high_temp} Precipitation: {self.precipitation} Snow: '
                f'{self.snowfall} Rain: {self.rainfall}')
...
Rain: 0
        ...
        #             f'{key}: {DailyWeather.__str__(self._records[key])}')
    ...
    def record_high(self, m: int, d: int) -> int:
        ...
        for dat in self._records:
            if dat.month == m and dat.day == d:
                if self._records[dat].high_temp > x:
                    x = self._records[dat].high_temp
        return x
    ...
    def monthly_average(self) -> Dict[str, float]:
        ...
        month_lst = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug',
                     'Sep', 'Oct', 'Nov', 'Dec']
        for key in month_lst:
            x[key] = 0
            y[key] = 0
            f[key] = None

        for d in self._records:
            x[d.strftime('%B')[0:3]] += 1
            y[d.strftime('%B')[0:3]] += self._records[d].low_temp

        for key2 in f:
            if x[key2] != 0:
                f[key2] = y[key2] / x[key2]
    ...
    def contiguous_precipitation(self) -> Tuple[date, int]:
        ...
        2
        """
        date_lst = sorted(self._records)
        value_lst = [self._records[x].precipitation for x in date_lst]
        ...
        i = 0
        # j = 0
        if not (value_lst[0] > 0 or value_lst[0] == -1):
            i = 1
        for j in range(1, len(date_lst)):
            if (not (value_lst[j] > 0 or value_lst[j] == -1)) or \
                    ((date_lst[j] - date_lst[j - 1]).days != 1):
                # not cts
                cur_len = j - i
                if cur_len > max_len:
                    max_len = cur_len
                ...
                i = j + 1
        # end one
        cur_len = len(date_lst) - i
        if cur_len > max_len:
            max_len = cur_len
            max_index = i

        # return tuple
        return (date_lst[max_index], max_len)
    ...
    def percentage_snowfall(self) -> float:
        ...
        total_rain = 0
        for key in self._records:
            if self._records[key].snowfall > 0:
                total_snow += self._records[key].snowfall
            if self._records[key].rainfall > 0:
                total_rain += self._records[key].rainfall
...
Rain: 0
        ...
        for location in self._histories:
            result2 += f'{HistoricalWeather.__str__(self._histories[location])}'
        return result2
    ...
    def snowiest_location(self) -> Union[Tuple[str, float], Tuple[None, None]]:
        ...
        result = [None, 0]
        for location in self._histories:
            t = HistoricalWeather.percentage_snowfall(self._histories[location])
            if t >= result[1]:
                result[0] = self._histories[location].name
                result[1] = t
        if result[0] is None:
            result[1] = None
...
def load_data(f: TextIO) -> Optional[HistoricalWeather]:
    ...
    try:
        ...
        station = str(data[0][2])
        hw = HistoricalWeather(str(data[0][2]),
                               (float(data[0][1]), float(data[0][0])))
    except ValueError:
        # contain no line
        return None
    ...
    for data_arr in data:
        if str(data_arr[2]) != station:
            continue
        # Basic info
        try:
            # create a dw
            dw = DailyWeather((float(data_arr[13]), float(data_arr[11]),
                               float(data_arr[9])),
                              (float(data_arr[23]) if
                               data_arr[24] != 'T' else -1,
                               float(data_arr[19]) if
                               data_arr[20] != 'T' else -1,
                               float(data_arr[21]) if
                               data_arr[22] != 'T' else -1))

            # add dw to hw
            hw.add_weather(date(int(data_arr[5]),
                                int(data_arr[6]),
                                int(data_arr[7])), dw)
            add_num += 1
        except ValueError:
